...
| Tax-1 | Implemented on `claude/tax-1-implementation` — Nordic V1 rate seed (SE/NO/DK/FI) + builtin TaxProvider with collectMode + EU reverse-charge (intra-EU Q7) + long-stay rate=0 audit (Q5) + banker's rounding per line × jurisdiction + calculateTax orchestrator with 3-tier fallback (Decision 10) + auto-registration of builtin on module load (Q1) + 12-decision parity test file. Verified locally: tsc baseline 3 (no new), vitest +98 net new (32 seed-rates + 26 builtin + 15 calculate + 25 parity), eslint 0, npm run build TypeScript compiles. ⚠ Operator must fact-check Nordic rate-table before merge (Q10). |
| Tax-2 | Implemented on `claude/tax-2-impl` — DraftOrder pricing pipeline wired to Tax-1 calculator. tax-request.ts mapper (lineType → TaxCategory + nights→long-stay), fulfillment-country resolver (Tenant.addressCountry → "SE"), orchestrator + preview-totals call calculateTax() with discount-adjusted bases, freezePrices snapshots TaxLine rows in tx (deleteMany+createMany idempotent per Q6), convert.ts reparents TaxLine draftLineItemId→orderLineItemId UPDATE-in-place per Q7 (preserves audit history). Decisions implemented: #1 #2 #4 #11 #12. Verified locally: tsc baseline 3 (no new), vitest +57 net new (19 mapper + 6 country + 8 orch + 12 freeze + 8 reparent + 4 prior orch updates + 12 parity). Existing 634 draft-order tests green (3 assertion updates + sub-mock extensions for tenant/product/tenantTaxConfig/taxLine reads). Accommodation.taxRate kept as LEGACY (Q10). Pre-Tax-2 frozen-draft backfill deferred to Tax-3 (Q8). |
