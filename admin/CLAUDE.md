...
**If you change a field's validation rule:** mirror the change on
both sides in the same commit. CI will catch drift but that's the
backstop, not the workflow.

**Deletion condition:** if a future Zod 4.x release tree-shakes
cleanly to ≤25 KB gzipped, delete every `.validator.ts` and the
parity test, and switch the worker back to importing Zod schemas
directly. Verify with `npm run build:analytics-runtime` after the
upgrade.
