...
describe("capturePortalScreenshots", () => {
  it("calls goto with the correct URL", async () => {
    await capturePortalScreenshots("https://test.bedfront.com/preview/home", "tenant-1");
    expect(mockGoto).toHaveBeenCalledWith("https://test.bedfront.com/preview/home", {
  ...
  it("sets desktop viewport to 1440×900", async () => {
    await capturePortalScreenshots("https://test.bedfront.com", "tenant-1");
  ...
  it("always closes browser even on error", async () => {
    ...
    await expect(
      capturePortalScreenshots("https://test.bedfront.com", "tenant-1"),
    ).rejects.toThrow("timeout");
    expect(mockClose).toHaveBeenCalledTimes(1);
  });

  it("returns Buffer instances", async () => {
    const result = await capturePortalScreenshots("https://test.bedfront.com", "tenant-1");
