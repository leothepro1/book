...
import { searchGuestsForPicker } from "../new/actions";
...
export type ExistingDiscount = {
  ...
  targetedProducts: { id: string; title: string; imageUrl?: string | null }[];
  targetedCollections: { id: string; title: string; imageUrl?: string | null }[];
  targetedSegments: { id: string; name: string }[];
  targetedCustomers: { id: string; label: string }[];
};
...
export default function DiscountForm({
  ...
  // Form fields — initialized from discount if editing
  const [method, setMethod] = useState<Method>(discount?.method ?? "CODE");
  const [title, setTitle] = useState(discount?.title ?? "");
  const [description, setDescription] = useState(discount?.description ?? "");
  const [valueType, setValueType] = useState<ValueType>(discount?.valueType ?? "PERCENTAGE");
  const [valueInput, setValueInput] = useState(discount ? (discount.valueType === "PERCENTAGE" ? String(discount.value / 100) : String(discount.value / 100)) : "");
  const [codeInput, setCodeInput] = useState(discount?.codes[0]?.code ?? "");
  const [conditions, setConditions] = useState<Condition[]>(discount?.conditions ?? getInitialConditions(discountType));
  const [combinesProduct, setCombinesProduct] = useState(discount?.combinesWithProductDiscounts ?? false);
  const [combinesOrder, setCombinesOrder] = useState(discount?.combinesWithOrderDiscounts ?? false);
  const [hasUsageLimit, setHasUsageLimit] = useState(discount?.usageLimit != null);
  const [oncePerCustomer, setOncePerCustomer] = useState(discount?.conditions.some((c) => c.type === "ONCE_PER_CUSTOMER") ?? false);
  const [usageLimit, setUsageLimit] = useState(discount?.usageLimit ? String(discount.usageLimit) : "");
  const [startDate, setStartDate] = useState(discount?.startsAt ? new Date(discount.startsAt).toISOString().slice(0, 10) : "");
  const [startTime, setStartTime] = useState(discount?.startsAt ? new Date(discount.startsAt).toISOString().slice(11, 16) : "");
  const [endDate, setEndDate] = useState(discount?.endsAt ? new Date(discount.endsAt).toISOString().slice(0, 10) : "");
  const [endTime, setEndTime] = useState(discount?.endsAt ? new Date(discount.endsAt).toISOString().slice(11, 16) : "");
  const [hasEndDate, setHasEndDate] = useState(!!discount?.endsAt);
  ...
  const [eligibility, setEligibility] = useState<Eligibility>(
    discount?.appliesToAllCustomers === false
      ? (discount.targetedSegments.length > 0 ? "specific_segments" : "specific_customers")
      : "all_customers",
  );
  ...
  const [eligibilityItems, setEligibilityItems] = useState<EligibilityItem[]>(
    discount?.targetedSegments.map((s) => ({ id: s.id, label: s.name }))
    ?? discount?.targetedCustomers
    ?? [],
  );
  ...
  const [appliesTo, setAppliesTo] = useState<AppliesTo>(
    discount?.appliesToAllProducts === false
      ? (discount.targetedCollections.length > 0 ? "specific_collections" : "specific_products")
      : "all",
  );
  const [pickedItems, setPickedItems] = useState<PickedItem[]>(
    discount?.targetedProducts.map((p) => ({ id: p.id, title: p.title, imageUrl: p.imageUrl ?? null }))
    ?? discount?.targetedCollections.map((c) => ({ id: c.id, title: c.title, imageUrl: c.imageUrl ?? null }))
    ?? [],
  );
      ...
      <div className="admin-editor">
        ...
        <div className="admin-header pf-header">
          <h1 className="admin-title" style={{ display: "flex", alignItems: "center", gap: 0 }}>
            ...
            <span style={{ marginLeft: 3 }}>{isEdit ? title || discount!.title : "Skapa rabatt"}</span>
          </h1>
        </div>
