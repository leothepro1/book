...
import { REGISTER_STRINGS, type RegisterLocale } from "./locales";
import { LOGIN_LANGUAGES } from "../login/locales";
...
export default function RegisterForm({ tenantName, logoUrl, logoWidth, privacyHtml }: RegisterFormProps) {
  ...
  const [locale, setLocale] = useState<RegisterLocale>("sv");
  const rootRef = useRef<HTMLDivElement>(null);

  const t = REGISTER_STRINGS[locale];
  ...
  const content = step === "otp" ? (
    ...
    <>
      ...
      <p className="otp-login__switch">
        {t.hasAccount}{" "}
        <a href="/login" className="otp-login__switch-link">{t.hasAccountLink}</a>
      </p>
    </>
  );
  ...
  return (
    <div ref={rootRef} className="otp-login__layout">
      ...
      <LanguagePanel
        ...
        currentLang={locale}
        onSelect={(code) => { setLocale(code as RegisterLocale); setLangOpen(false); }}
      />
...
// ── Error slide (same pattern as login) ────────────────────────
